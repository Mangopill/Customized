...
@Mod.EventBusSubscriber(modid = Customized.MODID, value = Dist.CLIENT)
public class PropertyValueTooltip {
    ...
    public static void onItemTooltip(final ItemTooltipEvent event) {
        ...
        @Nonnull PropertyValue propertyValue = PropertyValueUtil.getPropertyValue(stack, player.level());
        ...
        if (SHOW_ESTIMATED_VALUE_TOOLTIP.get()) {
            MutableComponent estimatedComponent = Component.translatable("tooltip." + Customized.MODID + ".estimated_value",
                    ...
                    foodProperty.getNutrition(), foodProperty.getSaturationModifier()).withStyle(ChatFormatting.GREEN);
            event.getToolTip().add(estimatedComponent);
        }
        if (SHOW_ESTIMATED_BUFF_TOOLTIP.get()) {
            if (!foodProperty.getEffects().isEmpty()) {
                foodProperty.getEffects().forEach(buff -> {
                    MutableComponent estimatedBuff = Component.translatable("tooltip." + Customized.MODID + ".estimated_buff",
                            ...
                            Component.translatable(buff.getFirst().getDescriptionId()),
                            buff.getFirst().getDuration()).withStyle(ChatFormatting.GREEN);
