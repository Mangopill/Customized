...
public class CHatLayerRenderer<T extends LivingEntity, M extends HumanoidModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (stack.getItem() instanceof CHatItem hat) {
            renderHatModel(poseStack, buffer, packedLight, this.getParentModel(), stack, hat.getScale(), hat.getTranslateY(), hat.getTexture());
        }
    }
}