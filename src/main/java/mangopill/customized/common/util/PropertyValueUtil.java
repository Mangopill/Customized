...
public final class PropertyValueUtil {
    ...
    public static PropertyValue getPropertyValue(ItemStack stack, Level level) {
        List<PropertyValueRecipe> recipeHolder = level.getRecipeManager().getRecipesFor(ModRecipeRegistry.PROPERTY_VALUE.get(), new SimpleContainer(stack), level);
        ...
        return recipeHolder.stream()
                ...
                .orElseGet(() -> {
                    ...
                    recipeHolder.forEach(valueRecipe ->
    ...
    public static FoodProperties getFoodPropertyByPropertyValue(Level level, List<ItemStack> stackList, boolean shardByConsumption) {
        ...
        List<com.mojang.datafixers.util.Pair<MobEffectInstance, Float>> foodEffect = new ArrayList<>();
        int nutritionValue = 0;
        float saturationValue = 0.0F;
        for (ItemStack stack : stackList) {
            @Nonnull PropertyValue propertyValue = getPropertyValue(stack, level);
            FoodProperties food = stack.getFoodProperties(null);
            if (food != null) {
                if (!food.getEffects().isEmpty()) {
                    foodEffect.addAll(food.getEffects());
                }
            }
            if (!propertyValue.isEmpty()) {
                ...
                if (food != null) {
                    nutritionValue += food.getNutrition() * stack.getCount();
                    saturationValue += food.getSaturationModifier() * stack.getCount();
                    continue;
                }
        ...
        if (shardByConsumption){
            ...
            if (nutritionValue != 0){
                ...
                builder.saturationMod(newSaturationValue.floatValue() * 12);
            } else {
                builder.saturationMod(saturationValue / consumptionCount);
            }
            if (!foodEffect.isEmpty()) {
                for (com.mojang.datafixers.util.Pair<MobEffectInstance, Float> pair : foodEffect) {
                    builder.effect(pair.getFirst(), Math.min(pair.getSecond(), 1.0F));
                }
            }
        } else {
            ...
            if (nutritionValue != 0){
                ...
                builder.saturationMod(saturationValue);
            }
            if (!foodEffect.isEmpty()) {
                for (com.mojang.datafixers.util.Pair<MobEffectInstance, Float> pair : foodEffect) {
    ...
    public static List<com.mojang.datafixers.util.Pair<MobEffectInstance, Float>> getCustomizedFoodEffectList(Map<NutrientCategory, Float> nutrientTotal){
    ...
    public static @Nonnull Map<NutrientCategory, Float> getFilteredNutrientTotal(Map<NutrientCategory, Float> nutrientTotal) {
    ...
    public static Map<NutrientBuff, Float> getCombinationBuffMap(Map<NutrientCategory, Float> filteredNutrientTotal) {
        ...
        nutrientTotal.forEach(nutrientBuff -> {
            if (nutrientBuff.getEffect() instanceof CombinationMobEffect combinationMobEffect) {
    ...
    public static float getShrinkNutrition(Map<NutrientCategory, Float> nutrientTotal) {
        ...
        for (NutrientBuff nutrient : buffMap.keySet()) {
            if (nutrient.getEffect() instanceof ShrinkNutritionMobEffect effect) {
    ...
    public static float getShrinkSaturation(Map<NutrientCategory, Float> nutrientTotal) {
        ...
        float i = 0.0F;
        for (NutrientBuff nutrient : buffMap.keySet()) {
            if (nutrient.getEffect() instanceof ShrinkSaturationMobEffect effect) {
    ...
    public static void addBuffToList(float nutrientValue, List<com.mojang.datafixers.util.Pair<MobEffectInstance, Float>> foodEffect, NutrientBuff nutrientBuff) {
        ...
        foodEffect.add(new com.mojang.datafixers.util.Pair<>(
                new MobEffectInstance(nutrientBuff.getEffect(), duration, Math.min((int) (nutrientValue / BUFF_AMPLIFIER.get()), 9))
                , Math.min(probability, 1.0F)));
    }
