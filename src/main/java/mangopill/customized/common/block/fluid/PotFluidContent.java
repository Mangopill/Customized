...
public class PotFluidContent extends CFluidContent<PotState, PotState> {
    ...
    public static PotFluidContent getContent(Level level, BlockPos pos) {
        if (!level.getBlockState(pos).getValue(AbstractPotBlock.LID).equals(PotState.WITHOUT_LID)){
            return new PotFluidContent(CBlockRegistry.CASSEROLE.get(), Fluids.WATER, FluidType.BUCKET_VOLUME, PotState.WITHOUT_LID);
        } else {
            return new PotFluidContent(CBlockRegistry.CASSEROLE.get(), Fluids.EMPTY, 0, null);
        }
    }

    public static PotFluidContent getContentForFill(Level level, BlockPos pos) {
        if (level.getBlockState(pos).getValue(AbstractPotBlock.LID).equals(PotState.WITHOUT_LID)){
            return new PotFluidContent(CBlockRegistry.CASSEROLE.get(), Fluids.WATER, FluidType.BUCKET_VOLUME, PotState.WITH_DRIVE);
        } else {
            return new PotFluidContent(CBlockRegistry.CASSEROLE.get(), Fluids.EMPTY, 0, null);
        }
    }
