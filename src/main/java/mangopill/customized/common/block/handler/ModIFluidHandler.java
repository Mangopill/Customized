...
public abstract class ModIFluidHandler<T extends Comparable<T>, V extends T> implements IFluidHandler {
    ...
    @Override
    public int fill(@Nonnull FluidStack fluidStack, @Nonnull FluidAction fluidAction) {
        if (fluidStack.isEmpty()) {
            return 0;
        }
        ModFluidContent<T, V> contents = getContentForFill();
        if (contents.getFluid() != Fluids.EMPTY && !fluidStack.getFluid().equals(contents.getFluid())) {
            return 0;
        }
    ...
    @Override
    public @Nonnull FluidStack drain(@Nonnull FluidStack fluidStack, @Nonnull FluidAction fluidAction) {
        if (fluidStack.isEmpty()) {
            return FluidStack.EMPTY;
        } else {
            return fluidStack.getFluid() == this.getContent().getFluid()
    ...
    @Override
    public @Nonnull FluidStack drain(int maxDrain, @Nonnull FluidAction fluidAction) {
