...
public class ModBlockRegistry {
    ...
    public static final Supplier<Block> SUSPICIOUS_DIRT = BLOCK.register(
            "suspicious_dirt", () -> new ModBrushableBlock(DIRT, BlockBehaviour.Properties.of().mapColor(MapColor.DIRT).instrument(NoteBlockInstrument.SNARE).strength(0.25F).sound(SoundType.SUSPICIOUS_GRAVEL).pushReaction(PushReaction.DESTROY),
                    SoundEvents.BRUSH_GRAVEL, SoundEvents.BRUSH_GRAVEL_COMPLETED));
    public static final Supplier<Block> SALT_PAN = BLOCK.register(
            "salt_pan", () -> new SaltPanBlock(BlockBehaviour.Properties.copy(OAK_PLANKS)));
    //kitchenware block
    ...
    public static final Supplier<Block> BREWING_BARREL = BLOCK.register(
            "brewing_barrel", () -> new BrewingBarrelBlock(Block.Properties.copy(BARREL)));
}
